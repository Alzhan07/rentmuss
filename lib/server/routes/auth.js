...
router.post('/register', registerLimiter, async (req, res) => {
  try {
    ...
    if (!name || !lastName || !password) {
      return res.status(400).json({ message: 'Имя, фамилия и пароль обязательны' });
    }
    ...
    const existing = await User.findOne({ name: nameNorm, lastName: lastNameNorm });
    if (existing) {
      return res.status(409).json({ message: 'Пользователь с таким именем и фамилией уже существует' });
    }
    ...
    console.log('✅ User зарегистрирован:', saved._id.toString(), saved.name, saved.lastName);
    ...
    if (err.code === 11000) {
      return res.status(409).json({ message: 'Имя, фамилия или email уже используется' });
    }
...
router.post('/login', async (req, res) => {
  try {
    ...
    const user = await User.findOne({ name: nameNorm, lastName: lastNameNorm });
    console.log('🔍 Found user:', user ? { id: user._id.toString(), name: user.name, lastName: user.lastName, role: user.role } : null);
